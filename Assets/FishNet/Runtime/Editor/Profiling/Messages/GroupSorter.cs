...
namespace Fishnet.NetworkProfiler.ModuleGUI.Messages
{
    internal struct GroupSorter
    {
        ...
        private readonly Func<PacketInfo, PacketInfo, int> _sortMessageFunc;
        ...
        public static int Compare<T>(PacketInfo x, PacketInfo y, Func<PacketInfo, T> func) where T : IComparable<T>
        {
            var xValue = func.Invoke(x);
            var yValue = func.Invoke(y);
            return xValue.CompareTo(yValue);
        ...
        public static int DefaultMessageSort(PacketInfo x, PacketInfo y)
        {
            return x.Order.CompareTo(y.Order);
        }
    }
