...
namespace FishNet.CodeGenerating.Processing.Rpc
{
    internal class RpcProcessor : CodegenBase
    {
        ...
        private bool CreateClientRpcWriterMethod(List<ParameterDefinition> serializedParameters, List<AttributeData> attributeDatas, CreatedRpc cr)
        {
            ...
            {
                string context = $"ClientRPC parameter '{serializedParameters[i].Name}' in method '{cr.OriginalMethodDef.DeclaringType.FullName}.{cr.OriginalMethodDef.Name}()'";
                MethodReference writeMethodRef = wp.GetOrCreateWriteMethodReference(serializedParameters[i].ParameterType, context);
        ...
        private bool CreateServerRpcWriterMethod(List<ParameterDefinition> serializedParameters, CreatedRpc cr)
        {
            ...
            for (int i = 0; i < serializedParameters.Count; i++)
            {
                string context = $"ServerRPC parameter '{serializedParameters[i].Name}' in method '{cr.OriginalMethodDef.DeclaringType.FullName}.{cr.OriginalMethodDef.Name}()'";
                MethodReference writeMethodRef = wp.GetOrCreateWriteMethodReference(serializedParameters[i].ParameterType, context);
                if (writeMethodRef == null)
                    return false;
