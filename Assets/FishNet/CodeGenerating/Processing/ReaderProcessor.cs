...
namespace FishNet.CodeGenerating.Helping
{
    ...
    internal class ReaderProcessor : CodegenBase
    {
        ...
        /// <summary>
        /// Returns the read MethodRef for typeRef. If read method doesn't exist then one will be created.
        /// </summary>
        internal MethodReference GetOrCreateReadMethodReference(TypeReference typeRef)
        {
            return GetOrCreateReadMethodReference(typeRef, null);
        }

        /// <summary>
        /// Returns the read MethodRef for typeRef. If read method doesn't exist then one will be created.
        /// </summary>
        internal MethodReference GetOrCreateReadMethodReference(TypeReference typeRef, string context)
        {
...
#pragma warning restore CS0219
            ...
            if (readMethodRef == null)
            {
                LogDeserializationError(typeRef, context);
            }
            ...
            else if (typeRef.IsGenericInstance)
            {
                ...
                {
                    MethodReference result = GetOrCreateReadMethodReference(item, $"generic parameter of {typeRef.Name}{(context != null ? $" in {context}" : "")}");
                    if (result == null)
                    {
                        LogDeserializationError(item, $"generic parameter of {typeRef.Name}{(context != null ? $" in {context}" : "")}");
                        return null;
                    }
                }
            }
        ...
        /// <summary>
        /// Logs a detailed deserialization error with context information and clickable file location.
        /// </summary>
        private void LogDeserializationError(TypeReference typeRef, string context)
        {
            string baseError = $"Could not create deserializer for {typeRef.FullName}.";
        ...
        private string FindTypeUsageLocation(TypeReference typeRef, string context)
        {
            ...
            if (context != null && (context.Contains("SyncVar") || context.Contains("field")))
            {
